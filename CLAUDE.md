...
| working on the Word add-in | [`docs/WORD-ADDIN.md`](docs/WORD-ADDIN.md) — what it is physically, the three hosts, how it is installed and tested, and what the store needs — then `apps/word-addin/` |
...
**And the Word add-in has a gate of its own, for the same reason.**
`apps/word-addin/src/word/client.ts` cannot be unit-tested — there is no
headless Word, and `office-addin-mock` does not mock collections — so
`npm run check:word:live` drives a REAL Word over COM. `Range.InsertXML` and
`Range.WordOpenXML` are the same two operations as `insertOoxml` and
`getOoxml`, over the same flat OPC package. It is not in `npm run check`
...
corrupting the owner's own file: a raised reading aid spliced into the
recitation because the style sheet did not define the style the body named;
26 of his 872 paragraphs invisible to our OOXML reader, which made every
index in a whole-document re-mark wrong; a paragraph replacement eating its
empty neighbour. See [`docs/WORD-ADDIN.md`](docs/WORD-ADDIN.md).

**And the landing page has a gate now, because it has cost twice.**
`CHROME=<path> npm run check:site` reads `site/` from the FILES — so it fails
before a push rather than after one — and looks at all three pages at 390, 768
and 1280 px: every image has real pixels in it, every internal link and anchor
goes somewhere, and nothing sticks out or is clipped. Both faults it exists for
were invisible in the source — `shots/` in `.gitignore` had no leading slash so
