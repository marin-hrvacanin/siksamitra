...
class FileOperations {
    ...
    setupEventListeners() {
        ...
        // Do not use beforeunload prompts (they trigger native browser dialogs).
    }
    ...
    async openFile(file) {
        try {
            ...
            // Show blocking loader immediately (prevents visual glitching on large files)
            ...
            if (titleEl) titleEl.textContent = 'Opening File';
            if (msgEl) msgEl.textContent = 'Please wait while the file is being loaded…';
            if (overlay && popup) {
            ...
            // Load via Quill clipboard parsing so custom formats are preserved.
            this.quill.setText('', 'silent');
            this.quill.clipboard.dangerouslyPasteHTML(0, content, 'silent');
            // Re-apply any special post-processing formats if the main editor is available.
            if (window.siksamitraEditor) {
                try {
                    window.siksamitraEditor.applySvaraFormattingInRange(0, window.siksamitraEditor.quill.getLength());
                    window.siksamitraEditor.applyDirghaFormattingInRange(0, window.siksamitraEditor.quill.getLength());
                    window.siksamitraEditor.updateButtonStates(window.siksamitraEditor.quill.getSelection(true));
                } catch (_) {
                    // Ignore if not available; content is still loaded.
                }
            }
...
<body>
     ...
     */
    async confirmSave() {
        // Map to tri-state: true(save) / false(discard) / null(cancel)
        if (!window.ModalDialogs) {
            return false;
        }
        const choice = await ModalDialogs.saveChanges(this.currentFileName);
        if (choice === 'save') return true;
        if (choice === 'dont-save') return false;
        return null;
    }
