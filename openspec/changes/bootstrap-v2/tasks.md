...
- [ ] 4.12 **One page map for the screen, the PDF and the `.docx`.** `paginate` has ONE caller — `PagedView.tsx`. The PDF is the HTML export put through the host browser's own print pagination and the `.docx` is paginated by Word, so the program has three page mechanisms and no test comparing them; `paginate.ts` and `PagedView.tsx` both claimed for a long time that the view and the export shared one map and that an integration test proved it, and neither was true. What holds them together today is a rule rather than a map: a verse is not split unless it cannot fit a page alone, which is `w:keepLines` in Word, `break-inside: avoid` in `export.css` and `mayBreak` in the view. Measured before that: the view split 22 of Śrī Rudram's verses at A4 that both exports keep whole. The real fix is for the exporters to place blocks from the same map, which needs measurement outside a live browser.
